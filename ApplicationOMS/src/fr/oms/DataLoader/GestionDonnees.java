...
public class GestionDonnees implements iAccesDonnees{
	...
	public void lireDonnees() {
		...
		Personne personne = new Personne(0,"Monsieur", "Dubois", "Michel", "63 Place de roudoux", "03278", "Clermont Ferrand","dubois.michel@oragne.fr", "0470569847","0645879869");
		List<Equipement> mesEquipements = new ArrayList<Equipement>();
		Equipement equipement = new Equipement(0, "Salle 1", "", null);
		mesEquipements.add(equipement);
		equipement = new Equipement(1, "Gymnase 1", "", null);
		mesEquipements.add(equipement);
		Association association = new Association(0,"PEUGEOT SPORT CLERMONT-FERRAND",false,mesEquipements,"10h à 12h",null,personne);
		Manager.getInstance().getListeAssociation().add(association);
		association = new Association(2,"CM POLICE NATIONALE",true,mesEquipements,"10h à 12h",null,personne);
		Manager.getInstance().getListeAssociation().add(association);
		association = new Association(3,"FANNY SAINT JACQUES",true,mesEquipements,"10h à 12h",null,personne);
		Manager.getInstance().getListeAssociation().add(association);
		association = new Association(4,"EASY RIDE (SKATE)",true,mesEquipements,"10h à 12h",null,personne);
		Manager.getInstance().getListeAssociation().add(association);
		association = new Association(5,"ACS EMIGRANTS PORTUGAIS DU PDD / Sect° Foot",true,mesEquipements,"10h à 12h",null,personne);
		Manager.getInstance().getListeAssociation().add(association);
		association = new Association(6,"USC LOGICA AUVERGNE",true,mesEquipements,"10h à 12h",null,personne);
		Manager.getInstance().getListeAssociation().add(association);
		association = new Association(7,"SNOOK & POOL",true,mesEquipements,"10h à 12h",null,personne);
		Manager.getInstance().getListeAssociation().add(association);
		association = new Association(8,"AS MONTFERRAND (Football)", true,mesEquipements,"10h à 12h",null,personne);
		Manager.getInstance().getListeAssociation().add(association);
		association = new Association(9,"ACTIV' RANDOMES",false,mesEquipements,"10h à 12h",null,personne);
		Manager.getInstance().getListeAssociation().add(association);
		association = new Association(10,"SQUASH DES VOLCANS",true,mesEquipements,"10h à 12h",null,personne);
		Manager.getInstance().getListeAssociation().add(association);
		
		List<Equipement> lesEquipements = new ArrayList<Equipement>();
		equipement = new Equipement(0, "Salle 1", "", null);
